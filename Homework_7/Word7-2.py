...
while t1[-1] <= 60:
    if abs(theta1[-1])>=pi:
        ...
        theta1.append(theta1[-1] + (omega1[-1] - (g/length)*py.sin(theta1[-1])*deltat - q1*omega1[-1]*deltat + force1*py.sin(frequency1*t1[-1])*deltat)*deltat)
...
while t2[-1] <= 60:
    if abs(theta2[-1])>=pi:
        ...
        theta2.append(theta2[-1] + (omega2[-1] - (g/length)*py.sin(theta2[-1])*deltat - q2*omega2[-1]*deltat + force2*py.sin(frequency2*t2[-1])*deltat)*deltat)
...
while t3[-1] <= 60:
    if abs(theta3[-1])>=pi:
        ...
        theta3.append(theta3[-1] + (omega3[-1] - (g/length)*py.sin(theta3[-1])*deltat - q3*omega3[-1]*deltat + force3*py.sin(frequency3*t3[-1])*deltat)*deltat)
